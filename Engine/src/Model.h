...
public:
    ...
    //List<TextureData> textures_loaded;    // Stores all the textures loaded so far,
...
private:
    ...
    // Processes a node in a recursive fashion. Processes each individual flatShadingMesh located at the node and repeats this process on its children nodes (if any).
    void processNode(aiNode* node, const aiScene* scene)
    {
        // Process each flatShadingMesh located at the current node
    ...
    MeshData processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        // Walk through each of the flatShadingMesh's vertices
        for (GLuint i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            // Texture Coordinates
            if (mesh->mTextureCoords[0]) // Does the flatShadingMesh contain texture coordinates?
            {
                Vector2 vec;
        ...
        // Now wak through each of the flatShadingMesh's faces (a face is a flatShadingMesh its triangle) and retrieve the corresponding vertex indices.
        ...
        // Return a flatShadingMesh object created from the extracted flatShadingMesh data
        return MeshData(vertices,indices,textures);
    }

    // Checks all lampMaterial textures of a given type and loads the textures if they're not loaded yet.
...
GLint TextureFromFile(const char* path, std::string directory)
{
    ...
    if (!image) {
        LOG_ERROR("texture %s loading error ... \n", filename.c_str());
    }
    else LOG_ERROR("texture %s loaded\n", filename.c_str());
